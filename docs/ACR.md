...
- Content formats: Store Docker images, Helm charts, and OCI artifacts in a
  single registry.
- Layer sharing: ACR shares common layers across images, reducing storage costs
...
- lab/qa:
    - namespace: lab (used for development purposes)
    - example: <lab|qa>/azure-acr
...
- lab/qa:
    - semantic: 0.0.0-SNAPSHOT (same as app version)
...
- `$Registry` is substituted by ACR Tasks, not by the shell. Escape it (`\$`)
  so bash leaves it alone; inside double quotes bash would expand it.
