...
#
# Multi-stage Dockerfile for the azure-acr Spring Boot application.
#
#   This Dockerfile is not building the Spring Boot "uber or fat" jar.
#   The Spring Boot multi-layered package uber jar
#   "azure-acr-spring-boot.jar" file should be created from the
...
# Build example:
#
#   docker build --build-arg APP_VERSION=1.0.0 -t azure-acr:1.0.0 .
#
...
# azure-acr-spring-boot.jar Spring Boot uber file in the project
# app/build/libs folder. The "azure-acr-spring-boot.jar" is assumed
# to be a copy of the Spring Boot multi-layered jar file created during
# the "./gradlew build".
ARG JAR_FILE=app/build/libs/azure-acr-spring-boot.jar
...
LABEL org.opencontainers.image.title="azure-acr" \
      ...
      org.opencontainers.image.source="https://github.com/rubensgomes-org/azure-acr" \
