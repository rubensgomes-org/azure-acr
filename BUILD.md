...
How the Gradle build for `azure-acr` is put together, what each task
...
> **Note** — `logging.level.com.rubensgomes.azure.acr` is set to `trace` in the
...
```bash
kill $(pgrep -f com.rubensgomes.azure.acr.App)
```
...
```bash
./gradlew bootJar
java -jar app/build/libs/azure-acr-<version>.jar
```
...
```kotlin
excludes = listOf("com.rubensgomes.azure.acr.config.*")
```
...
| Archive                                  | Contents                                                 |
|------------------------------------------|----------------------------------------------------------|
| `azure-acr-<version>.jar`         | executable Spring Boot jar (layered, for Docker caching) |
| `azure-acr-<version>-plain.jar`   | library jar, classes only                                |
| `azure-acr-<version>-sources.jar` | sources                                                  |
| `azure-acr-<version>-javadoc.jar` | Javadoc                                                  |
...
| `sonar.projectKey`       | `rubensgomes-org_azure-acr` |
| `sonar.projectName`      | `azure-acr`                 |
